...
import type { QuarterlyContactResult } from '@/lib/hubspot/client'
...
function PacingTrackerQuarterly({
  current,
  baseline,
  baselineLabel,
  daysElapsed,
    ...
    <div className="rounded-md border border-white/[0.06] bg-black/20 px-5 py-4 space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-[11px] font-extrabold uppercase tracking-widest text-text-muted">
          Pacing · Day {daysElapsed} of {totalDays} · {quarterLabel}
        </p>
        {projDelta !== null && (
      ...
      <p className="text-[10px] text-text-muted">
        On pace for <span className="font-bold text-white">{Math.round(projected).toLocaleString()}</span> contacts this quarter
        {gapPerDay !== null && (
...
interface QuarterlyPacingProps {
  data: QuarterlyContactResult
}

export function QuarterlyPacing({ data }: QuarterlyPacingProps) {
  const {
    quarters,
    currentQuarterLabel,
    currentQuarterTotal,
  ...
  return (
    <div className="rounded-lg border border-white/[0.06] bg-bg-surface p-6 space-y-6">
      ...
      <div className="flex items-center gap-2">
        <h3 className="text-lg font-bold text-white">Quarterly Pacing</h3>
        <InlineTooltip text="Current-quarter contact creation vs. prior quarter and prior year same quarter. Pacing tracker shows progress against the previous quarter's final total." />
      </div>

      {/* Pacing tracker */}
      <PacingTrackerQuarterly
        current={currentQuarterTotal}
        baseline={prevQuarterTotal}
        baselineLabel="prev quarter"
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <StatCard
          ...
          value={currentQuarterTotal}
          tooltip={`Inbound contacts created so far in ${currentQuarterLabel}.`}
        />
        <StatCard
          title="Previous Quarter"
          value={prevQuarterTotal}
          delta={pct(currentQuarterTotal, prevQuarterTotal)}
          tooltip="Inbound contacts created in the previous full quarter. Delta shows how the current quarter compares on a paced basis."
        />
        <StatCard
          title="Prior Year Quarter"
          value={prevYearQuarterTotal}
          delta={pct(currentQuarterTotal, prevYearQuarterTotal)}
          tooltip={`Inbound contacts in the same quarter one year ago. Delta shows year-over-year change.`}
        />
      </div>
      ...
      {quarters.length > 1 && (
        <div className="space-y-3">
          {quarters.map((item) => {
            ...
            return (
              <div key={item.quarterKey} className="flex items-center gap-3">
                <div className="w-16 shrink-0 text-right text-xs font-semibold text-text-muted">
                  {item.label}
                </div>
