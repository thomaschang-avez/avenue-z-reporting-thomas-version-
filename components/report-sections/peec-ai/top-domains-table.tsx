        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="group relative flex-shrink-0">
            ...
            <span className="pointer-events-none absolute bottom-full left-1/2 z-10 mb-2 w-56 -translate-x-1/2 rounded-md border border-white/[0.08] bg-bg-surface px-3 py-2 text-xs leading-relaxed text-text-muted opacity-0 shadow-xl transition-opacity duration-150 group-hover:opacity-100">
              Top domains retrieved by AI models in their answers. Confirm with Tina Fleming how domains are added and managed in Peec.
              <span className="absolute left-1/2 top-full -translate-x-1/2 border-4 border-transparent border-t-white/[0.08]" />
            </span>
          </span>
        </div>
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-white/[0.04]">
            ...
            <ColHeader label="Retrieved"     tooltip="% of AI queries that retrieved this domain as a source." />
            <ColHeader label="Citation Rate" tooltip="How often the domain is cited when it is retrieved." />
        ...
        <tbody>
          {visible.map((d, i) => (
            ...
            >
              ...
              <td className="px-5 py-3 text-right">
                <span className="tabular-nums text-white">{d.citationRate.toFixed(1)}</span>
