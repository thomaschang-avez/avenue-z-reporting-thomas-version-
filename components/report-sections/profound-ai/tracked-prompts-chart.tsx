  ...
  return (
    <>
      ...
      >
        ...
        <td className="px-5 py-3 text-right"><MetricCell value={group.avgVisibility} suffix="%" /></td>
        <td className="px-5 py-3 text-right"><MetricCell value={group.avgSov} suffix="%" /></td>
        <td className="px-5 py-3 text-right"><MetricCell value={group.avgPosition} suffix="" prefix="#" /></td>
      </tr>
      {open && group.prompts.map((p) => (
        <tr key={p.text} className="border-b border-white/[0.03] bg-white/[0.01] hover:bg-white/[0.02] transition-colors">
          ...
          <td className="px-5 py-2.5 text-right"><MetricCell value={p.visibility} suffix="%" /></td>
          <td className="px-5 py-2.5 text-right"><MetricCell value={p.sov} suffix="%" /></td>
          <td className="px-5 py-2.5 text-right"><MetricCell value={p.position} suffix="" prefix="#" /></td>
        </tr>
      ))}
    </>
  )
          ...
          <p className="text-xs text-text-muted flex items-center gap-1">
            ...
            <span className="group relative flex-shrink-0">
              ...
              <span className="pointer-events-none absolute bottom-full right-0 z-10 mb-2 w-52 rounded-md border border-white/[0.08] bg-bg-surface px-3 py-2 text-[11px] font-normal normal-case leading-relaxed tracking-normal text-text-muted opacity-0 shadow-xl transition-opacity duration-150 group-hover:opacity-100">
                Total number of unique prompts tracked in Profound across all AI models and channels.
